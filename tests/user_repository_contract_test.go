...
func TestUserRepository_Create_DuplicateEmail(t *testing.T) {
	...
	// Create first user
	user1, err := domain.NewUser(siteID, "user@example.com", "encrypted_password_hash_that_is_at_least_sixty_characters_long")
	require.NoError(t, err)
	err = repo.Create(ctx, user1)
	require.NoError(t, err)

	// Try to create second user with same email in same site
	user2, err := domain.NewUser(siteID, "user@example.com", "different_password_hash_that_is_at_least_sixty_characters_long")
	require.NoError(t, err)
